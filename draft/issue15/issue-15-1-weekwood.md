...
开始测试之旅并不是一件轻松的事，尤其是在没有人帮助的情况下。如果之前你曾经做过类似的尝试，当你想起的时候或许你记得那么一个时刻：“就是它，我现在就要开始测试。我听闻 TDD 是多么有益，所以我现在要这么做。”

Then you sat down in front of your computer. You opened your IDE. You created a new test file for one of your components.

然后你坐在电脑前，打开 IDE, 为你的 components 建立了第一个测试文件。
...
然后它作废了，也许你写了一些基本功能的测试用例，但是你总觉得哪里不对。有个问题始终潜伏在脑海深处。在真正开始之前有一个问题必须要回答。
...
要回答这个问题并没有那么简单，实际上。这是一个非常复杂的问题。值得清醒的是你不是第一个有如此疑问的人，也不是最后一个。
...
这些测试有一个根本的问题：它们不会告诉你会发生什么。它们不会实际的预期。它不*清楚*需求到底是什么。
...
此外，当一个测试失败，你必须深入代码并且*理解*为什么失败。这需要大量不必要的认知负荷。
...
这就是行为驱动开发 (BDD)，它旨在解决具体问题，帮助开发人员确定*应该测试什么*。此外，它提供了一个DSL（译者注: Domain-specific language，域特定语言）鼓励开发者*理清*他们的需求，并且它引入了一个通用语言帮助您轻易*理解*测试的目的是什么
...
如此深刻的问题的答案却惊人的简单，但是它需要改变你的对测试套件的看法。BDD 的第一个单词就表明了这一点，你不应该关注于*测试*，而是应该关注*行为*。这个看似毫无意义的变化提供了，应该测试什么的准确答案：你应该测试行为。
...
在我们讨论 BDD DSL 好处之前，让我们首先通过基础功能来看一个 `Car` 类的简单测试套件：
...
可能你已经注意到，几乎所有的组件定义了 DSL 由两部分组成：一个字符串值定义了什么被测试，以及一个块测试本身或者多个组件。这些字符串有两个非常重要的功能。
...
首先，在 `describe` 块内，这些字符串组绑定到一个测试功能特定的一部分（例如，移动一辆汽车）。因为你可以尽可能多的指定你希望的嵌套块。你可以在对象中或者它们的依赖关系中编写基于上下文的不同规范。
...
这就是正在发生在 `move to` 的事情：`describe` 块：我们建立了两个  `context`  块来提供 `Car` 内基于不同状态的不同期望（发动机启动或关闭）。这就是例子，说明了 BDD DSL 鼓励*理清*给定对象应该如何在给定的条件下表现的需求。
...
接下来，这些字符串被用来创建哪些测试失败的句子。例如，让我们假设引擎启动的测试用例失败了。我们将收到 `Car move to when engine is not running should not move to given position` 的错误信息。这些语句对我们理解失败和预期行为提供了非常大的帮助，重点是不需要阅读任何实际代码，因为他们减少了认知负荷。此外，它提供了一个标准语言来帮助你了解你团队的每一个成员，即便他们技术略差。
...
记住你可以编写不包含 BDD-style 语法有着明确需求和易于理解命名的测试用例（例如 XCtest）。然而，BDD 已经建立从头建立了这些功能和语法，使得测试更佳容易。
...
对于 iOS 或者 Mac 开发者，你应该从多样的 BDD 框架中选择一个：
...
当涉及到语法，所有这些框架几乎是相同的。它们之间的主要区别在于他们的可配置性和绑定组件。
...
**Cedar** 捆绑了 [matchers](https://github.com/pivotal/cedar/wiki/Writing-specs#matchers) 和 [doubles](https://github.com/pivotal/cedar/wiki/Writing-specs#doubles)。虽然这不完全真实，为了这篇文章。让我们考虑 doubles 作为 mocks (你可以在[这篇文章中] (/issue-15-5) 学习 doubles 和 mocks 的区别)
...
除了这些 helpers，Ceder 包含了额外的配置功能：集中测试。集中测试的意思是 Ceder 将只执行一个测试或者一组测试。主要可以通过添加在  `it`，`describe` 或者 `context` 前面添加  `f`，
...
同样 Ceder 提供了相反的配置能力：你可以添加 `x` 到测试中来关闭它。XCTest 有类似的配置能力，然而他们是通过操作 schemes 实现 (或者手动点击 "Run this test")。Ceder 配置能力更简单快速去配置。
...
Cedar 用了一点黑客技术才能与 XCTest 集成，如果 Apple 决定改变 XCTest 内部实现的话，那么 Cedar 非常容易失灵，然而从用户角度来看， Cedar 工作起来就像集成 XCTest 一样。
...
正如前面提到过的，Cedar，Kiwu，以及 Specta 提供类似语法，我不能说其中一个框架要比其他所有都好；它们各有利弊。选择一个 BDD 框架归根结底来自个人偏好。

It is also worth mentioning that there are already two BDD frameworks that are dedicated to Swift:

值得一提的是已经有两个 Swift 专用的 BDD 框架。
...
还有最后一件事我想在举例前指出。记住，编写好的行为测试代码最重要的方面是识别依赖关系（你可以在[这里](/issue-15/dependency-injection.html)阅读更多相关主题）
...
大部分你的测试将断言一个特定交互的发生，或者一个特定值是否返回（或者传递给另一个对象），基于你测试对象的状态。提取依赖允许你轻松 mock 值或者状态。此外，它将大大简化维护是否一个特定动作的发生或者特定值被计算。
...
记住，你不应该将你对象*所有*的依赖关系和属性暴露在接口之中（特别是当你开始测试的时候，虽然这样很诱人）。这将减少你对象的可读性和目的是否清晰，而你的界面应该清楚的表述设计需求。
...
请注意，这里（或者本文中其他例子）采用了 mocking 框架。如果你之前没有用过 mocking 框架，你应该向[这篇文章](/issue-15/mocking-stubbing.html)请教。
...
我们将先 mocking 我们这个组件内时间格式化的这个唯一依赖，我们将用创建的 mock 来返回开始和结束日期的固定字符串。然后我们检查从事件格式化程序构造返回的字符串是否和我们先前 mocked 有一样的值。
...
尽管我们有了完整的测试组件，我们写了一个非常小的测试，不是吗？让我们从一个稍微不同的角度来尝试解决这个问题。
...
上面这个例子并没有确切测试 `EventDescriptionFormatter` 的*行为*。它主要通过 mocking `NSDateFormatter` 来测试其内部实现。我们实际上并不关心是否下面有日期格式化程序。从一个接口角度来看，我们可以一直使用日期组件手动格式化日期。此时,我们关心重点是我们是否正确获取字符串。这个行为是我们需要测试的。
...
下一步当然是重构我们的测试。现在我们不再需要知道事件内部的formatter，我们只需要专注于实际的行为：
...
注意我们测试变的多么简单。我们仅仅有一个简约的设置块来准备数据模型和调用测试方法。更多的专注于行为测试的结果，而不是它实际工作方式，我们简化测试套件，同时仍然保留对我们对象的功能覆盖。这正是 BDD 想尝试思考行为的结果，而不是实际实现。
...
And of course of course, the interface for our network layer: 
...
这部分相当简单，下一步是检查在我们调用取消方法之后是否请求被取消。我们需要确定在没有标识符的时候不调用取消方法。规范这种行为看起来像这样：
...
你大概可以衡量这些测试中有一些错误。尽管对于检查特定行为这样是有效的，它们暴露了 `CalendarDataDownloader` 内部的工作。这里不需要有知识来测试 `CalendarDataDownloader`  如何持有它的请求标识符。让我们看看我们如何在不暴露我们内部实现的情况下描述我们的测试：
...
我们通过 stubbing `makeRequest:completion:` 方法开始。我们返回一个固定的标识符。在相同的 `describe` 块内，我们定义一个 取消的 `describe` 块，用以在`CalendarDataDownloader` 类中调用 `cancel` 方法。接着我们检查我们的固定字符串是否传入到我们 mocked 的网络层中的 `cancelRequestWithIdentifier:` 方法。
...
请注意，在这一点上我们没有实际测试网络请求是否被执行 - 我们不会得到一个标识符并且 `cancelRequestWithIdentifier:`  永远不会被调用。然而，我们保留测试来确保我们知道功能破坏了什么。
...
我们已经设法测试与暴露 `CalendarDataDownloader` 内部实现相同的行为。此外，我们用三个测试代替了之前四个。我们利用 BDD DSL 嵌套能力来束缚仿真的多重行为 - 我们第一次模拟下载，接下来，在相同的 `describe` 块内，我们模拟取消请求。
...
看来，在 iOS 开发者中对于测试视图控制器的最常见的态度看不到价值所在。这让我觉得奇怪，控制器经常代表应用程序的核心。他们是帮助所有组件粘合在一起。它们建立了用户见面和应用逻辑和模型之间的联系。因此，无意间的变化可能造成巨大的破坏。
...
让我们看下我们如何测试这个组件，首先，我们需要断言这个我们的 bar 按钮是否初始化了 title，target 和 action 属性：
...
但是这仅仅完成了我们一半的测试功能：我们现在需要确保当按钮按下的时候适当的方法被执行，但是我们无法确定适当的动作被执行（我们实际上也不知道这个方法有没有被创建）。接下来让我们开始测试：
...
让我们回头看看 `PhotoUploadViewController` 而且看看如何重写我们的测试确保我们只测试我们的界面，而不是实现。
...
首先，我们不需要知道 `didTapUploadButton:` 方法存在与否。它只是实现细节。我们应该只关心行为：当用户点击上传按钮，`UploadManager` 应该收到一个 `uploadPhoto:` 消息。非常好，意思说我们不再需要在 `PhotoUploadViewController` 中添加 `Specs` 类别
...
现在我们已经创建了 helper 方法模拟点击，我们可以在最上级的 `describe` 块中简化：
...
值得注意的是我们设法消除了两个测试后我们仍然包含一个严格测试的组件。此外，我们的测试套件不易被打破，我们不再依赖于 `didTapUploadButton:` 方法。最后但同样重要的，我们更关注我们控制器行为方面，而不是它的内部实现。
...
我们将通过构建一个 `SignInViewController` 包含两个文本框以及一个登录按钮。我们应该确保我们的控制器尽可能的轻量级，所以我们将抽象一个类负责登录到称为  `SignInManager` 的单独的组件中。
...
例子中提到的代码有一些问题。首先我们暴露了过多 `SignInViewController` 内部实现，包括按钮，文本框以及方法。事实是,我们并不真正需要做所有这一切。
...
让我们看看如何重构这些测试来确保没有出碰到内部实现。我们将通过删除登录按钮的 target 和 method 的钩子来开始:
...
现在我们可以调用这个按钮的方法和断言 `SignInManager` 是否收到相应的消息。但是我们已然可以改善这个测试的写法。
...
让我们假设我们不想知道谁拥有这个登录按钮，也许这是一个直接视图控制器的视图的子视图。或者我们封装在单独的视图包含自己的代理。我们实际上不应该关系它在哪；我们应该只关心它在我们视图控制器视图的某个地方并且当我们点击时候发生什么。我们可以用 helper 方法来抓取登录按钮，不用关心它在哪：
...
我们的方法将遍历视图中的所有子视图并返回第一个和我们 title 参数匹配的第一个按钮，我们可以写类似的方法用于文本字段或标签:
...
看来是不是更简单？注意它通过 title 来寻找按钮，我们同样也要测试这个按钮是否被创建。此外，模拟一个点击请求，我们测试动作是否被正确连接。而且最后，断言 `SignInManager` 应该被调用，我们测试了这部分有没有正确被实施 - 以上所有通过三个测试来实现。
...
另外非常棒的这些测试我们利用了 BDD DSL 的功能。注意我们使用 `context` 块为 `SignInViewController` 根据不同的需求行为来定义其文本字段状态。
...
行为驱动开发看起来并不像最初那么困难。所有你需要的只是改变你的思维方式 - 思考更多一个对象的表象（它的接口应该如何）并且减少对实现的关注。通过这样做，你将拥有更健壮的代码。以及同样伟大的测试套件。此外，你的测试变的不那么容易被打破重构，它们将专注于测试对象而不是内部实现。
...
并且 iOS 社区提供了极好的工具，你应该立即开始对于你 app 的行为驱动开发。现在你知道应该测试什么，不需要借口，不是吗？
