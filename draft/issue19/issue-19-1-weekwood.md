...
没人可以写完美的代码，但是调试我们都能做到。我将带你亲身经历一个 Bug 修复的过程，而不是提供一个关于这个话题的随机列表，回归 UIKit， 展示我用来理解，隔离病最终解决这个问题。
...
我收到了一个 bug 反馈报告，当快速点击一个按钮来弹出一个 popover 并隐藏 popover 但是会**推出**一个新视图控制器。谢天谢地，一个样本已经形成，所以第一步 -- 重现 Bug -- 我们已经估计到：
...
我的第一个猜测是，我们可能包含了隐藏视图控制器的代码，我们错误的关闭了他们的父类。然而，当使用 Xcode 集成的试图调试功能，很明显有一个全局 `UIDimmingView` 来相应点击事件：

![](http://img.objccn.io/issue-19/issue-19/xcode-view-debugging.png)
...
苹果在 Xcode 6 中添加了 [调试试图层次结构](https://developer.apple.com/library/ios/recipes/xcode_help-debugger/using_view_debugger/using_view_debugger.html)，这一举动很可能是收到了非常受欢迎的应用  [Reveal](http://revealapp.com/) 和 [Spark Inspector](http://sparkinspector.com/) 的启发。在许多方面，依然更好，比 Xcode 有更丰富的功能。
...
相似的检查视图层次结构，我们也可以用 `po [[[UIWindow keyWindow] rootViewController] _printHierarchy]` 来检查视图控制器。这是一个苹果默默加在 iOS 8 中的[私有帮助方法](https://github.com/nst/iOS-Runtime-Headers/blob/a8f9f7eb4882c9dfc87166d876c547b75a24c5bb/Frameworks/UIKit.framework/UIViewController.h#L365) 。
...
我通常用他来检查[响应链](https://developer.apple.com/library/ios/documentation/EventHandling/Conceptual/EventHandlingiPhoneOS/event_delivery_responder_chain/event_delivery_responder_chain.html)，虽然你可以手动循环对你感兴趣的对象执行`nextResponder`，或者[添加一个类别 helper](https://gist.github.com/n-b/5420684)，输入 `presponder object` 是迄今为止最快的方法。
...
我们首先要找出实际隐藏我们试图控制器的代码。最明显的作用是在 `viewWillDisappear:` 设置一个断点来进行堆栈跟踪：
...
看看这个堆栈，我们注意到视图控制器已经被隐藏，但我们已经执行了预定的动画，所以我们需要在更早的地方增加断点。在这个例子中，我们兴趣在于调用 `-[UIViewController dismissViewControllerAnimated:completion:]`。我们添加*符号断点* 到 Xcode 的断点列表，并且重新执行示例代码。
...
现在如我们所说！正如预期的，全屏 `UIDimmingView` 接收到我们的触摸并且在 `handleSingleTap:` 中处理，接着转发到 `UIPopoverPresentationController` 中的 `dimmingViewWasTapped:` 方法来隐藏视图控制器（就像它该做的那样），无论如何。当我们快速点击，断点调用两次。这里有第二个 dimming 视图？他们调用了相同的实例？我们只是组装这个断点，所以调用 `po self` 是无效的。
...
装配和function-calling惯例的一些基本知识，我们依然可以拿到 `self`  的值。[iOS ABI Function Call Guide](http://developer.apple.com/library/ios/#documentation/Xcode/Conceptual/iPhoneOSABIReference/Introduction/Introduction.html) 和 [Mac OS X ABI Function Call Guide](http://developer.apple.com/library/mac/#documentation/DeveloperTools/Conceptual/LowLevelABI/000-Introduction/introduction.html) 都是很好在 iOS 中使用的资源。
...
我们知道每个 Objective-C 有两个隐式参数： `self` 和 `_cmd`。所以我们需要的是第一个对象在堆栈上。在 **32-bit** 架构中，堆栈信息保存在 `$esp`，所以在 Objective- C 方法中你可以你可以使用 `po *(int*)($esp+4)` 来获取 `self`，以及使用 `p (SEL)*(int*)($esp+8)` 来获取 `_cmd`。`$esp` 的第一个值是返回地址。随后变量保存在 `$esp+12`, `$esp+16`, 等等。
...
**Arm64** 类似于 armv7，无论如何，有了更多的寄存器。整个范围 `$x0` 到 `$x7` 用来存放变量，之前回落到堆栈寄存器 `$sp`。
...
你可以学到更多关于[x86](http://eli.thegreenplace.net/2011/02/04/where-the-top-of-the-stack-is-on-x86/)的堆栈布局，以及  [x86-64](http://eli.thegreenplace.net/2011/09/06/stack-frame-layout-on-x86-64/)，同时可以阅读  [AMD64 ABI Draft](http://www.x86-64.org/documentation/abi.pdf)。
...
跟踪方法执行的另一种做法是在调用超类之前重写方法并加入日志。不管怎样，手动 swizzling 调试起来更加方便但是效率不高。话说，我写了一个很小库叫做 [*Aspects*](http://github.com/steipete/Aspects) 确实做到了这件事。它可以用于生产模式代码，但是我大部分时候用于调试和写测试用例。（如果你好奇 Aspects，你可以[在这里了解更多.](https://speakerdeck.com/steipete/building-aspects))
...
为 `dimmingViewWasTapped:` 添加钩子，他是私有方法 — 我们使用 `NSSelectorFromString`。你需要验证方法是否存在，并通过使用 [iOS Runtime Headers](https://github.com/nst/iOS-Runtime-Headers) 查找每个框架类的其他私有方法和公共方法。This project uses the fact that one can't really hide methods at runtime to query all classes and create a more complete header than what Apple gives us。（当然，调用私有 API 并不是一个好主意 — 这里只是用来便于理解事情如何发生）
...
同一时间，dimming 视图调用隐藏我们的主导航控制器。视图控制器的 `dismissViewControllerAnimated:completion:` 会转发视图控制器的解除请求到它的子视图控制器，如果只有一个将解除。所以第一次解除请求执行于popover，第二次，导航控制器本身被解雇。
...
现在我们知道发生了什么事情 — 所以我们可以进入*为何发生* 环节。UIKit 是闭源代码，但是我们使用反汇编就像 [Hopper](http://www.hopperapp.com/) 来解读 UIKit 组合并且仔细看看 `UIPopoverPresentationController` 发生了什么事情。你可以找到二进制文件 `/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator.sdk/System/Library/Frameworks/UIKit.framework`。使用 File -> Read Executable to Disassemble... 然后使用 Hopper，看他如何爬过二进制并且 symbolicates 代码。 32-bit  反汇编是最成熟的一个。所以你选择 32-bit 文件可以拿到最好的结果。[IDA by Hex-Rays](https://www.hex-rays.com/products/ida/) 是另一个很强大很昂贵的反汇编程序，通常提供[甚至更好的结果](https://twitter.com/steipete/status/537565877332639744):
...
装配的一些基本阅读代码时非常有用。然而,你也可以使用伪代码视图来得到更多的东西 C-like:
...
阅读伪代码是令人膛目的。有两个代码路径 — 其中一个是代理方法实现`popoverPresentationControllerShouldDismissPopover:`，如果不，代码路径实际上相当不同。而一个反应基本代表代理方法包含 `if (controller.presented && !controller.dismissing)`，另一个代码路径（我们实际落入），总是被解雇。通过内部信息，我们可以尝试通过实现我们自己的 `UIPopoverPresentationControllerDelegate` 来解决这个 bug：
...
现在请不要停下。我们通常需要使用文档内的方法来解决问题，相当重要的是，写一个bug report 给 Apple，作为额外好处，这样你能够真正理解错误，并且没有其他副作用在你程序中发挥作用 — 如果你放弃一个 iOS 版本，很容易测试 radar 是否有效。
...
写一个 Radar 实际上是非常有趣的挑战，不像你想象的那么多时间。用一个示例，您将帮助一些劳累苹果工程师，没有它，工程师将最有可能推迟，甚至不考虑雷达。我设法创建一个示例在大约50 LOC，包括一些意见和解决方案。一个视图模板通常是最快的方法创建一个示例。
...
现在，我们知道了苹果的RadarWeb 并没有想象的那么好，然而你可以不使用它。[QuickRadar](http://www.quickradar.com/) 是一个有非常优秀 Mac 前端来提交 radar，同时提交一个副本到 [OpenRadar](http://openradar.appspot.com)。此外,它使欺骗雷达极其方便。你应该马上下载它,欺骗rdar:/ / 19053416如果你觉得这样的错误应该是固定的。
...
并不是所有问题都可以用一些简单的解决方案，这些步骤将帮助您找到更好的解决问题,或者至少提高你的理解为什么有些事情正在发生。
